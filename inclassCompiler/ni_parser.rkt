...
(define niparser
  (parser
   ...
   (end EOF)
   (tokens value-tokens paren-types operators punctuation comparators boolops keywords endoffile test-tokens)
...
|#
    ...
    (thingsWithEnd
     ...
     [(LET declarations IN) (NoVal)] ; Said this does not yield a value... so assume just ignore all of it...
     ...
     [(IF expr THEN expr ELSE expr ) (IfThenExpr $2 $4 $6)] ; if then else statement.
